...
import {format, parseISO, startOfMonth, endOfMonth, startOfWeek, endOfWeek, eachDayOfInterval} from "date-fns";
...
export default function InternshipHistory({userId, canReview=false}: {userId:string; canReview?:boolean}) {
  ...
  // Carrega sempre a semana selecionada para popular o dropdown de dias
  useEffect(() => {
    const loadWeekDays = async () => {
      try {
        const result = await getHistory({ userId, mode: "week", date: selectedWeek });
        setWeekDays(result.data.days);
        // opcional: se estiver em modo week, usar esses dados
        if (mode === "week") setData(result.data);
      } catch (e) {
        console.error(e);
      }
    };
  ...
  const current = mode === "day" ? (daysByDate.get(selectedDay) ?? weekDays.find(d => d.date === selectedDay)) : undefined;
