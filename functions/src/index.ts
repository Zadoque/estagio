...
function getClientIp(request: functions.https.CallableRequest): string {
...
function requireSupervisor(request: functions.https.CallableRequest): string {
...
export const createInternshipInvite = functions.https.onCall(async (request) => {
  ...
  if (!name || !email || !/^\d{4}$/.test(pin) || !/^\d{4}-\d{2}-\d{2}$/.test(startDate)) {
    throw new functions.https.HttpsError("invalid-argument", "Nome, e-mail, PIN de 4 digitos e data de inicio sao obrigatorios.");
  }
